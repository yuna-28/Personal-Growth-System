...
想找人互相監督時用。在 Apps Script 的 `VIEW_CODE` 填一串**跟 APP_CODE 不一樣**的密碼，重新部署（記得選「新版本」）。把這組密碼＋你的 app 網址給朋友：她在設定頁輸入後會進入「監督模式」——看得到你所有紀錄和統計，但畫面上的編輯功能全部隱藏，而且就算繞過畫面，後端也會拒絕任何寫入。留空 `''` ＝不啟用。
